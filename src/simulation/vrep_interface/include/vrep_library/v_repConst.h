...
{ /* Model properties (serialized): */
  ...
  sim_modelproperty_not_respondable = 0x0040,       /* cannot be selected if sim_modelproperty_not_dynamic is not
                                                       selected */
  sim_modelproperty_not_reset = 0x0080,             /* Model is not reset at simulation
                                                       end. This flag is cleared at
                                                       simulation end */
  sim_modelproperty_not_visible = 0x0100,           /* Whole model is invisible,
...
{ /* Check the documentation instead of comments below!! */
  ...
  sim_message_ui_button_state_change = 0, /* a UI button, slider, etc. changed (due to a user's action).
                                             aux[0]=UI handle, aux[1]=button handle, aux[2]=button
                                             attributes, aux[3]=slider position (if slider) */
  ...
  sim_message_keypress,      /* a key was pressed while the focus was on a page
                                (aux[0]=key, aux[1]=ctrl and shift key state) */
  sim_message_bannerclicked, /* a banner was clicked (aux[0]=banner ID) */
  sim_message_scene_loaded,  /* a scene was loaded */

  sim_message_prox_sensor_select_down, /* a "geometric" click select (mouse
                                          down) was registered. Enable with
                                          sim_intparam_prox_sensor_select_down.
                                          aux[0]=objectID, aux2[0-2]=pt
                                          coord, aux2[3-5]=pt normal vector
                                          */
  sim_message_prox_sensor_select_up,   /* a "geometric" click select (mouse
                                          up) was registered. Enable with
                                          sim_intparam_prox_sensor_select_up.
                                          aux[0]=objectID, aux2[0-2]=pt
                                          coord, aux2[3-5]=pt normal vector
                                          */
  sim_message_pick_select_down,        /* a "pick" click select (mouse down) was
                                          registered. aux[0]=objectID */

  /* Following messages are dispatched only to the C-API (not available
     from Lua): */
  ...
  sim_message_eventcallback_modelsave,    /* about to save a model (current
                                             selection will be saved) */
  sim_message_eventcallback_moduleopen,   /* called when simOpenModule in Lua
                                             is called */
  sim_message_eventcallback_modulehandle, /* called when simHandleModule in
                                             Lua is called with argument
                                             false */
  sim_message_eventcallback_moduleclose,  /* called when simCloseModule in
                                             Lua is called */
  ...
  sim_message_eventcallback_instancepass, /* Called once every main
                                             application loop pass.
                                             auxiliaryData[0] contains
                                             event flags of events that
  ...
  /* bit7 set: edit mode active. This is not an event flag, but a state
     flag */
  /* bit8 set: object(s) scaled */
  /* bit9 set: selection state changed. (different objects are selected
     now) */
  ...
  sim_message_eventcallback_abouttoundo,        /* the undo button was hit and a
                                                   previous state is about to be
                                                   restored */
  sim_message_eventcallback_undoperformed,      /* the undo button was hit and a
                                                   previous state restored */
  sim_message_eventcallback_abouttoredo,        /* the redo button was hit and a
                                                   future state is about to be
                                                   restored  */
  sim_message_eventcallback_redoperformed,      /* the redo button was hit and a
                                                   future state restored  */
  sim_message_eventcallback_scripticondblclick, /* scipt icon was double
                                                   clicked.  (aux[0]=object
                                                   handle associated with
                                                   script, set replyData[0]
                                                   to 1 if script should
                                                   not be opened)  */
  ...
  sim_message_eventcallback_keypress,                  /* a key was pressed while the focus
                                                          was on a page (aux[0]=key,
                                                          aux[1]=ctrl and shift key state)
                                                          */
  sim_message_eventcallback_modulehandleinsensingpart, /* called when
                                                          simHandleModule
                                                          in Lua is called
                                                          with argument
                                                          true */

  sim_message_eventcallback_renderingpass,    /* called just before the scene
                                                 is rendered. From the UI
                                                 thread! */
  sim_message_eventcallback_bannerclicked,    /* called when a banner was
                                                 clicked (aux[0]=banner ID) */
  sim_message_eventcallback_menuitemselected, /* auxiliaryData[0] indicates
                                                 the handle of the item,
                                                 auxiliaryData[1] indicates
                                                 the state of the item */
  sim_message_eventcallback_refreshdialogs,   /* aux[0]=refresh degree
                                                 (0=light, 1=medium, 2=full)
                                                 */
  ...
  sim_message_eventcallback_rmlposition, /* the command simRMLPosition was
                                            called. The appropriate plugin
                                            should handle the call */
  sim_message_eventcallback_rmlvelocity, /* the command simRMLVelocity was
                                            called. The appropriate plugin
                                            should handle the call */

  sim_message_eventcallback_reserved18,
  sim_message_eventcallback_reserved19,
  sim_message_eventcallback_pathplanningplugin, /* to interact with the
                                                   path planning plugin */
  sim_message_eventcallback_colladaplugin,      /* to interact with the collada
                                                   plugin */

  sim_message_eventcallback_opengl,           /* a simple callback at different
                                                 rendering stages */
  sim_message_eventcallback_openglframe,      /* a callback with the full
                                                 rendered opengl frame data
                                                 (that can be modified then
                                                 returned) */
  sim_message_eventcallback_openglcameraview, /* a callback with the
                                                 rendered opengl view data
                                                 (that can be modified then
                                                 returned) */

  sim_message_eventcallback_proxsensorselectdown, /* a "geometric" click
                                                     select (mouse down)
                                                     ...
                                                     normal vector
                                                     (floats)*/
  sim_message_eventcallback_proxsensorselectup,   /* a "geometric" click
                                                     select (mouse up) was
                                                     registered. Enable with
                                                     sim_intparam_prox_sensor_select_down.
                                                     aux[0]=objectID,
                                                     customData[0-2]=pt coord
                                                     (floats),
                                                     customData[3-5]=pt
                                                     normal vector (floats)*/
  sim_message_eventcallback_pickselectdown,       /* a "pick" click select (mouse
                                                     down) was registered.
                                                     aux[0]=objectID */

  sim_message_eventcallback_rmlpos,    /* the command simRMLPos was called.
                                          The appropriate plugin should handle
                                          the call */
  sim_message_eventcallback_rmlvel,    /* the command simRMLVel was called.
                                          The appropriate plugin should handle
                                          the call */
  sim_message_eventcallback_rmlstep,   /* the command simRMLStep was called.
                                          The appropriate plugin should
                                          handle the call */
  sim_message_eventcallback_rmlremove, /* the command simRMLRemove was
                                          called. The appropriate plugin
                                          should handle the call */
  sim_message_eventcallback_rmlinfo,   /* used internally */

  sim_message_eventcallback_beforerendering, /* called just before the
                                                scene is rendered. From the
                                                main SIM thread! */
...
{ /* DEPRECATED, check below */
  ...
  sim_lua_arg_double,
  sim_lua_arg_table = 8
  /* SIM_LUA_ARG_NIL_ALLOWED=65536 is defined and used in
     CLuaFunctionData.h. This flag is reserved here. */
};
...
{ /* type of arguments (input and output) for custom script commands */
  ...
  /* SIM_SCRIPT_ARG_NULL_ALLOWED=65536 is defined and used in
     CScriptFunctionData.h. This flag is reserved here. */
};
...
{                                                       /* Customization script call types */
  ...
  sim_customizationscriptcall_cleanup,                  /* last time called. Do some
                                                           clean-up */
  sim_customizationscriptcall_nonsimulation,            /* called while simulation not
                                                           running */
  sim_customizationscriptcall_lastbeforesimulation,     /* called just before
                                                           simulation starts */
  sim_customizationscriptcall_firstaftersimulation,     /* called just after
                                                           simulation ended */
  sim_customizationscriptcall_simulationactuation,      /* called in the
                                                           "actuation" phase of
                                                           the main script.
                                                           Called only if the
                                                           ...
                                                           */
  sim_customizationscriptcall_simulationsensing,        /* called in the "sensing"
                                                           phase of the main
                                                           script. Called only if
                                                           the script is flagged
                                                           as
                                                           sim_customizationscriptattribute_activeduringsimulation
                                                           */
  sim_customizationscriptcall_simulationpause,          /* called while simulation
                                                           is paused */
  sim_customizationscriptcall_simulationpausefirst,     /* called just after a
                                                           simulation was
                                                           paused */
  sim_customizationscriptcall_simulationpauselast,      /* called just before a
                                                           simulation is
                                                           unpaused */
  sim_customizationscriptcall_lastbeforeinstanceswitch, /* called just
                                                           before an
                                                           instance switch
                                                           (switch to
                                                           another scene)
                                                           */
  sim_customizationscriptcall_firstafterinstanceswitch  /* called just after
                                                           an instance
                                                           switch (switch to
                                                           another scene) */
};
...
{                                                              /* Script attributes */
  ...
  sim_scriptattribute_executionorder,                          /* integer attribute, see
                                                                  sim_scriptexecorder_normal, etc.
                                                                  */
...
{ /* Generic dialog styles: */
  ...
  sim_dlgstyle_yes_no,
  sim_dlgstyle_dont_center = 32 /* can be combined with one of above
                                   values. Only with this flag can the
                                   position of the related UI be set just
...
{ /* drawing objects: */
  ...
  sim_drawing_trianglepoints, /* 6 values per point (3 for triangle
                                 position, 3 for triangle normal vector)
                                 (triangle size in meters) */
  sim_drawing_quadpoints,     /* 6 values per point (3 for quad position, 3 for
                                 quad normal vector) (quad size in meters) */
  sim_drawing_discpoints,     /* 6 values per point (3 for disc position, 3 for
                                 disc normal vector) (disc size in meters) */
  sim_drawing_cubepoints,     /* 6 values per point (3 for cube position, 3 for
                                 cube normal vector) (cube size in meters) */
  sim_drawing_spherepoints,   /* 3 values per point (sphere size in meters)
                                 */

  /* following can be or-combined: */
  sim_drawing_itemcolors = 0x000020,             /* +3 values per item (each item has
                                                    its own ambient color (r,g,b
                                                    values)). Mutually exclusive with
                                                    sim_drawing_vertexcolors */
  sim_drawing_vertexcolors = 0x000040,           /* +3 values per vertex (each vertex has its own ambient
                                                    color (r,g,b values). Only for sim_drawing_lines (+6)
                                                    and for sim_drawing_triangles(+9)). Mutually exclusive
                                                    with sim_drawing_itemcolors */
  sim_drawing_itemsizes = 0x000080,              /* +1 value per item (each item has its own size). Not for
  ...
  sim_drawing_painttag = 0x000400,               /* all items are tagged as paint (for
                                                    additinal processing at a later
                                                    stage) */
  sim_drawing_followparentvisibility = 0x000800, /* if the object is associated with a scene object, then it
                                                    follows that visibility, otherwise it is always visible
                                                    */
  sim_drawing_cyclic = 0x001000,                 /* if the max item count was reached, then the first items
                                                    are overwritten. */
  ...
  sim_drawing_emissioncolor = 0x010000,          /* When used in combination with
                                                    sim_drawing_itemcolors or
                                                    sim_drawing_vertexcolors, then
                                                    the specified colors will be for
                                                    the emissive component */
  sim_drawing_facingcamera = 0x020000,           /* Only for trianglepoints,
                                                    quadpoints, discpoints and
                                                    cubepoints. If specified, the
                                                    normal verctor is calculated to
                                                    face the camera (each item data
                                                    requires 3 values less) */
  sim_drawing_overlay = 0x040000,                /* When specified, objects are always
                                                    drawn on top of "regular objects" */
  sim_drawing_itemtransparency = 0x080000,       /* +1 value per item (each item has its own transparency
                                                    value (0-1)). Not compatible with
                                                    sim_drawing_vertexcolors */
  sim_drawing_persistent = 0x100000,             /* object is not automatically removed when created via a
                                                    simulation script */
  sim_drawing_auxchannelcolor1 = 0x200000,       /* when specifying the emission color component in the
                                                    simAddDrawingObject function, 6 values should be
                                                    specified instead of 3: 3 for the emission component,
                                                    and 3 for the aux channel component */
  sim_drawing_auxchannelcolor2 = 0x400000,       /* When used in combination with
                                                    sim_drawing_itemcolors or
                                                    sim_drawing_vertexcolors,
                                                    then the specified colors
                                                    will be for the aux channel
                                                    component */
};
...
{ /* banner values: */
  ...
  sim_banner_overlay = 0x00008,                /* When specified, banners are always drawn on top of
                                                  "regular objects" */
  sim_banner_followparentvisibility = 0x00010, /* if the object is
                                                  associated with a scene
                                                  object, then it follows
                                                  that visibility,
                                                  otherwise it is always
                                                  visible */
  sim_banner_clickselectsparent = 0x00020,     /* if the object is associated with a scene object, then
                                                  clicking the banner will select the scene object */
  sim_banner_clicktriggersevent = 0x00040,     /* if the banner is clicked, an event is triggered
                                                  (sim_message_eventcallback_bannerclicked and
                                                  sim_message_bannerclicked are generated) */
  sim_banner_facingcamera = 0x00080,           /* If specified, the banner will always face the camera by
                                                  rotating around the banner's vertical axis (y-axis) */
  sim_banner_fullyfacingcamera = 0x00100,      /* If specified, the banner will always fully face the
                                                  camera (the banner's orientation is same as the camera
                                                  looking at it) */
  sim_banner_backfaceculling = 0x00200,        /* If specified, the banner will
                                                  only be visible from one side */
  sim_banner_keepsamesize = 0x00400,           /* If specified, the banner will always appear in the same
                                                  size. In that case size represents the character height
                                                  in pixels */
  sim_banner_bitmapfont = 0x00800              /* If specified, a fixed-size bitmap font is used. The text
                                                  will also always fully face the camera and be right to the
                                                  specified position. Bitmap fonts are not clickable */
};

enum
{ /* particle objects: */
  /* following are mutually exclusive: */
  sim_particle_points1 = 0,  /* 6 values per point (pt1 and pt2. Pt1 is start position, pt2-pt1
                                is the initial velocity vector). Point is 1 pixel big. Only
                                appearance is a point, internally handled as a perfect sphere
                                */
  sim_particle_points2,      /* 6 values per point. Point is 2 pixel big. Only
                                appearance is a point, internally handled as a
                                perfect sphere */
  sim_particle_points4,      /* 6 values per point. Point is 4 pixel big. Only
                                appearance is a point, internally handled as a
                                perfect sphere */
  sim_particle_roughspheres, /* 6 values per sphere. Only appearance is
                                rough. Internally a perfect sphere */
  sim_particle_spheres,      /* 6 values per sphere. Internally a perfect sphere
                                */

  /* following can be or-combined: */
  sim_particle_respondable1to4 = 0x0020,     /* the particles are respondable against shapes (against all
                                                objects that have at least one bit 1-4 activated in the
                                                global respondable mask) */
  sim_particle_respondable5to8 = 0x0040,     /* the particles are respondable against shapes (against all
                                                objects that have at least one bit 5-8 activated in the
                                                global respondable mask) */
  sim_particle_particlerespondable = 0x0080, /* the particles are respondable against each other */
  sim_particle_ignoresgravity = 0x0100,      /* the particles ignore the effect
                                                of gravity. Not compatible with
                                                sim_particle_water */
  sim_particle_invisible = 0x0200,           /* the particles are invisible */
  sim_particle_itemsizes = 0x0400,           /* +1 value per particle (each particle
                                                can have a different size) */
  sim_particle_itemdensities = 0x0800,       /* +1 value per particle (each
                                                particle can have a different
                                                density) */
  sim_particle_itemcolors = 0x1000,          /* +3 values per particle (each
                                                particle can have a different color)
                                                */
  sim_particle_cyclic = 0x2000,              /* if the max item count was reached, then the first items
                                                are overwritten. */
  sim_particle_emissioncolor = 0x4000,       /* When used in combination with sim_particle_itemcolors,
                                                then the specified colors will be for the emissive
                                                component */
  sim_particle_water = 0x8000,               /* the particles are water particles (no weight in the water
                                                (i.e. when z<0)). Not compatible with
                                                sim_particle_ignoresgravity */
  sim_particle_painttag = 0x10000            /* The particles can be seen by vision
                                                sensors (sim_particle_invisible must
                                                not be set) */
};
...
enum
{                                     /* Integer parameters: */
  sim_intparam_error_report_mode = 0, /* Check sim_api_errormessage_...
                                         constants above for valid values
                                         */
  sim_intparam_program_version,       /* e.g Version 2.1.4 --> 20104. Can only be
                                         read. See also
                                         sim_intparam_program_revision */
  sim_intparam_instance_count,        /* do not use anymore (always returns 1
                                         since V-REP 2.5.11) */
  sim_intparam_custom_cmd_start_id,   /* can only be read */
  sim_intparam_compilation_version,   /* 0=evaluation version, 1=full
                                         version, 2=player version. Can only
                                         be read */
  ...
  sim_intparam_dynamic_engine,        /* 0=Bullet, 1=ODE. 2=Vortex. 3=Newton. Can
                                         only be read */
  ...
  sim_intparam_server_port_next,         /* is initialized at
  ...
  sim_intparam_program_revision, /* Can only be read. See also
  ...
  sim_intparam_scene_index, /* can be used to switch to a different
                               instance programmatically */
  sim_intparam_motionplanning_seed,
  sim_intparam_speedmodifier, /* can only be used while simulation is not
                                 stopped */
...
{                                       /* String parameters: */
  ...
  sim_stringparam_additional_addonscript_firstscene, /* do not use. Can
                                                        only be written. */
  sim_stringparam_additional_addonscript,            /* do not use. Can only be
                                                        written. */
...
{ /* Navigation and selection modes with the mouse. Lower byte values are
  ...
  /* Bit-combine following values and add them to one of above's values for
     a valid navigation mode: */
...
#define SIMX_VERSION 11 /* max is 255!!! */
                        ...
                        /* version to 10 for release AFTER 3.2.3. Added simxGetCollectionHandle and
                         * simxCallScriptFunction */
/* version to 11 for release AFTER 3.3.0. Added uses stacks for data exchange
 * with scripts */
...
#define simx_headeroffset_crc                                                                                          \
  0 /* 1 simxUShort. Generated by the client or server. The CRC for the \ \ \ \                                                                                                                     \
       message */
#define simx_headeroffset_version                                                                                      \
  2 /* 1 byte. Generated by the client or server. The version of the remote \  \                                       \
       \                                                                       \                                       \
       \ \                                                                             \                               \
       API software */
#define simx_headeroffset_message_id                                                                                   \
  3 /* 1 simxInt. Generated by the client (and used in a reply by the server)  \                                       \
     * \                                                                       \                                       \
     * \ \                                                                             \                               \
     * \ \ \                                                                             \                             \
       */
#define simx_headeroffset_client_time                                                                                  \
  7 /* 1 simxInt. Client time stamp generated by the client (and sent back by  \                                       \
       \                                                                       \                                       \
       \ \                                                                             \                               \
       \ \ \                                                                             \                             \
       the server) */
#define simx_headeroffset_server_time                                                                                  \
  11 /* 1 simxInt. Generated by the server when a reply is generated. The \ \  \                                       \
        \                                                                      \                                       \
        server timestamp */
#define simx_headeroffset_scene_id                                                                                     \
  15 /* 1 simxUShort. Generated by the server. A unique ID identifying the \ \ \                                       \
        \                                                                      \                                       \
        scene currently displayed */
#define simx_headeroffset_server_state                                                                                 \
  17 /* 1 byte. Generated by the server. Bit coded: 0 set --> simulation not \ \                                       \
        \                                                                      \                                       \
        \ \                                                                             \                              \
        stopped, 1 set --> simulation paused, 2 set --> real-time switch on, \ \                                       \
        \                                                                      \                                       \
        \ \                                                                             \                              \
        3-5: edit mode type (0=no edit mode, 1=triangle, 2=vertex, 3=edge, \ \ \                                       \
        \                                                                      \                                       \
        4=path, 5=UI) */

/* Remote API command header: */
#define SIMX_SUBHEADER_SIZE 26
#define simx_cmdheaderoffset_mem_size                                                                                  \
  0 /* 1 simxInt. Generated by the client or server. The buffer size of the \  \                                       \
       \                                                                       \                                       \
       \ \                                                                             \                               \
       command. */
#define simx_cmdheaderoffset_full_mem_size                                                                             \
  4 /* 1 simxInt. Generated by the client or server. The full buffer size of \ \                                       \
       \                                                                       \                                       \
       \ \                                                                             \                               \
       the command (applies to split chunks). */
#define simx_cmdheaderoffset_pdata_offset0                                                                             \
  8 /* 1 simxUShort. Generated by the client or server. The amount of data \ \ \                                       \
       \                                                                       \                                       \
       that is part of the command identification. */
#define simx_cmdheaderoffset_pdata_offset1                                                                             \
  10 /* 1 simxInt. Generated by the client or server. The amount of shift of \ \                                       \
        \                                                                      \                                       \
        \ \                                                                             \                              \
        the pure data buffer (applies to split chunks). */
#define simx_cmdheaderoffset_cmd                                                                                       \
  14 /* 1 simxInt. Generated by the client (and used in a reply by the \ \ \ \                                                                                                                     \
        server). The command, combined with the operation mode of the command. \                                       \
        \                                                                      \                                       \
        \ \                                                                             \                              \
        \ \ \                                                                             \                            \
        */
#define simx_cmdheaderoffset_delay_or_split                                                                            \
  18 /* 1 simxUShort. Generated by the client or server. The amount of delay \ \                                       \
        \                                                                      \                                       \
        \ \                                                                             \                              \
        in ms of a streaming command, or the max. pure data size to send at \  \                                       \
        \                                                                      \                                       \
        \ \                                                                             \                              \
        once (applies to split commands). */
#define simx_cmdheaderoffset_sim_time                                                                                  \
  20 /* 1 simxInt. Generated by the server. The simulation time (in ms) when \ \                                       \
        \                                                                      \                                       \
        \ \                                                                             \                              \
        the command was executed (or 0 if simulation is not running) */
#define simx_cmdheaderoffset_status                                                                                    \
  24 /* 1 byte. Generated by the server. (1: bit 0 is set --> error in \ \ \ \                                                                                                                     \
        function execution on server side). The client writes bit 1 if command \                                       \
        \                                                                      \                                       \
        \ \                                                                             \                              \
        \ \ \                                                                             \                            \
...
enum
{
  ...
  /* from here on, commands are also identified by 4 additional bytes and 2
     additional strings */
  ...
  simx_opmode_oneshot = 0x000000,      /* sends command as one chunk. Reply will also come as one
                                          chunk. Doesn't wait for the reply. */
  simx_opmode_blocking = 0x010000,     /* sends command as one chunk. Reply will
                                          also come as one chunk. Waits for the
                                          reply (_REPLY_WAIT_TIMEOUT_IN_MS is the
                                          timeout). */
  simx_opmode_oneshot_wait = 0x010000, /* same as simx_opmode_blocking */
  simx_opmode_streaming = 0x020000,    /* sends command as one chunk. Command will
                                          be stored on the server and always
                                          executed (every x ms (as far as
                                          possible), where x can be 0-65535. just
                                          add x to simx_opmode_streaming). A reply
                                          will be sent continuously, each time as
                                          one chunk. Doesn't wait for the reply. */
  simx_opmode_continuous = 0x020000,   /* same as simx_opmode_streaming */

  /* Operation modes for heavy data */
  simx_opmode_oneshot_split = 0x030000,    /* sends command as several chunks (max chunk size is x bytes,
                                              where x can be _MIN_SPLIT_AMOUNT_IN_BYTES-65535. Just add x
                                              to simx_opmode_oneshot_split). Reply will also come as
                                              several chunks. Doesn't wait for the reply. */
  simx_opmode_streaming_split = 0x040000,  /* sends command as several chunks (max chunk size is x bytes,
                                              where x can be _MIN_SPLIT_AMOUNT_IN_BYTES-65535. Just add x
                                              to simx_opmode_streaming_split). Command will be stored on
                                              the server and always executed. A reply will be sent
                                              continuously, each time as several chunks. Doesn't wait for
                                              the reply. */
  simx_opmode_continuous_split = 0x040000, /* same as simx_opmode_streaming_split */

  /* Special operation modes */
  simx_opmode_discontinue = 0x050000, /* removes and cancels all commands of the given type stored on
                                         the client or server side (also streaming commands) */
  simx_opmode_buffer = 0x060000,      /* doesn't send anything, but checks if a reply for the given
                                         command is available in the input buffer (i.e. previously
                                         received from the server) */
  simx_opmode_remove = 0x070000       /* doesn't send anything and doesn't return any specific value.
                                         It just erases a similar command reply in the inbox (to free
                                         some memory) */
};
...
enum
{
  ...
  simx_return_novalue_flag = 0x000001,         /* input buffer doesn't contain the
                                                  specified command. Maybe you forgot to
                                                  enable data streaming, or streaming
                                                  hasn't started yet */
  simx_return_timeout_flag = 0x000002,         /* command reply not received in time for simx_opmode_blocking
                                                  operation mode */
  ...
  simx_return_split_progress_flag = 0x000010,  /* previous similar command not yet fully processed (applies to
                                                  simx_opmode_oneshot_split operation modes) */
...
/* Following only for backward compatibility, but equivalent to above return
 * values */
enum
{
...
enum
{
  ...
  simros_strmcmd_get_info,  // do not use. Is streamed anyway with topic name
                            // "info"
  ...
  /* from here on, commands are also identified by one additional int and one
     additional string */
