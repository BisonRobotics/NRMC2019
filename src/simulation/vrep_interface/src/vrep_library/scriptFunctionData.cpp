...
#include "../../include/vrep_lib/scriptFunctionData.h"
#include <sstream>
#include <cstring>
...
{  // use this when reading data from a
   // script from inside of a custom script
   // function call
  return (&_inData);
}
...
{  // use this when reading data from a script from
   // inside of a custom script function call
  bool retVal = _readData(stackHandle, expectedArguments, requiredArgumentCount, functionName, "arguments.",
                          "Argument ", _inData);
  simPopStackItem(stackHandle, 0);  // clears the stack (which will serve as
                                    // return value container from now)
  return (retVal);
}
...
{  // use this when reading data returned from a
   // Lua function call from a plugin
...
{  // use this when returning data
   // from inside of a custom Lua
   // function call
...
{  // use this when returning data from inside of a custom
   // Lua function call
...
{  // use this when reading
  ...
  for (int i = 0; i < argCnt; i++)
  {
    ...
    if (simIsStackValueNull(stack) == 1)
    {
      ...
      {  // yes. This is for an argument that
         // can optionally also be nil.
      ...
      {  // no
        ...
        break;  // this argument is nil, so it is like inexistant. But we also
                // won't explore any more arguments, we have enough.
      }
    }
    if (!done)
    {
