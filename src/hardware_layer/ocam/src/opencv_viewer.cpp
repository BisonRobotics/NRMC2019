/*
 * The Example of the ocam oCam-1MGN-U API using with OpenCV. (Linux only.)
 *
 * This example shows how to get image from the oCam-1MGN using the ocam camera API.
 * And also shows how to control the oCam-1MGN using the ocam camera API.
 *
...
{ 
    ...
    camera.set_format(1280, 720, ocam::fourcc_to_pixformat('G','R','E','Y'), 1, 50);

    /* 8-bit Greyscale 1280 x 960 45 fps */
    //camera.set_format(1280, 960, ocam::fourcc_to_pixformat('G','R','E','Y'), 1, 45);

    /* 8-bit Greyscale 320 x 240 160 fps */
    //camera.set_format(320, 240, ocam::fourcc_to_pixformat('G','R','E','Y'), 1, 160);

    /* 8-bit Greyscale 640 x 480 80 fps */
    //camera.set_format(640, 480, ocam::fourcc_to_pixformat('G','R','E','Y'), 1, 80);

    /* USB 2.0 */
    /* 8-bit Greyscale 1280 x 720 30 fps */
    //camera.set_format(1280, 720, ocam::fourcc_to_pixformat('G','R','E','Y'), 1, 30);

    /* 8-bit Greyscale 1280 x 960 22.5 fps */
    //camera.set_format(1280, 960, ocam::fourcc_to_pixformat('G','R','E','Y'), 2, 45);
    ...
    while (!quit) {
      ...
      t_now = Clock::now();
      double diff = (double)std::chrono::duration_cast<std::chrono::microseconds>(t_now - t_last).count();
      avg_freq = 1e6/diff;
      t_last = t_now;
      //printf("Freq: %f\n", avg_freq);
