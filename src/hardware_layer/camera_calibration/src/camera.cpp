...
using namespace ocam;

#define OCAM_CAMERA_IOCTL_RETRY     5
#define OCAM_CAMERA_REQUEST_BUFFER_COUNT   5
...
bool Camera::set_buffer()
{
    ...
    v4l2_s.requestbuffers.count = OCAM_CAMERA_REQUEST_BUFFER_COUNT;
...
bool Camera::enumerate_frame_sizes(const unsigned int pixelformat, std::string& description)
{
    ...
    while (xioctl(VIDIOC_ENUM_FRAMESIZES, &v4l2_s.frmsizeenum) != -1) {
        ...
        case V4L2_FRMSIZE_TYPE_DISCRETE:
            ...
            desc.clear();
            desc = description + (" " + ocam::to_string<int>(v4l2_s.frmsizeenum.discrete.width) + " x " + ocam::to_string<int>(v4l2_s.frmsizeenum.discrete.height));
...
bool Camera::enumerate_frame_intervals(const unsigned int pixelformat, const unsigned int width, const unsigned int height, std::string& description)
{
    ...
    while (xioctl(VIDIOC_ENUM_FRAMEINTERVALS, &v4l2_s.frmivalenum) != -1) {
        ...
        case V4L2_FRMIVAL_TYPE_DISCRETE:
            ...
            desc.clear();
            desc = description + (" " + ocam::to_string<double>(fps) + " fps");
...
int Camera::xioctl(int IOCTL_X, void *arg)
{
    ...
    int ret = 0;
    int tries= OCAM_CAMERA_IOCTL_RETRY;
    do
    {
        if(!disable_libv4l2)
    ...
    if (ret && (tries <= 0)) {
        DBG_PRINTF("V4L2_CORE: ioctl (%i) retried %i times - giving up: %s)", IOCTL_X, OCAM_CAMERA_IOCTL_RETRY, strerror(errno));
    }
